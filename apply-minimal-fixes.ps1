# Apply minimal fixes to preserve original structure while making images work
$categories = @(
...
foreach ($category in $categories) {
    ...
    if (Test-Path $file) {
        ...
        # Only add a minimal script that fixes the image loading without changing structure
        ...
        # Add minimal script at the end
        $minimalScript = "<script>
// Fix image loading for file:// protocol
setTimeout(function() {
...
$imageTags
        ...
        $content = $content -replace '</body></html>', $minimalScript
        
        Set-Content $file $content -NoNewline
        Write-Host "Applied minimal fix to: $category"
    } else {
        Write-Host "File not found: $file"
    }
}

Write-Host "Minimal fixes applied - original structure preserved!"
