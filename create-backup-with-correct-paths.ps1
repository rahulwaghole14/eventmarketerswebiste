# Create backup with correct image paths
$categories = @(
...
foreach ($category in $categories) {
    ...
    if (Test-Path $file) {
        # Create backup
        Copy-Item $file "$file.backup-final" -Force
        
        $content = Get-Content $file -Raw
        
        # Remove any existing manual image loading script
        ...
        foreach ($img in $images) {
            ...
            if (Test-Path $imgPath) {
                ...
                $imageTags += "                    <img src='../../market_brand_logo.png' alt='Fallback Image' class='w-full h-48 object-cover rounded-lg shadow-lg hover:scale-105 transition-transform'>`n"
            }
        }
        ...
        # Add robust manual image loading script
        $script = "<script>
// Manual image loading for file:// protocol - BACKUP VERSION
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() {
        const loadingDiv = document.querySelector('.min-h-[400px].flex.items-center.justify-center');
        if (loadingDiv) {
            console.log('Loading images for $displayName...');
...
$imageTags
            ...
            console.log('Images loaded successfully for $displayName');
        } else {
            console.log('Loading div not found for $displayName');
        }
    }, 1500);
});
</script>
        ...
        Write-Host "Created backup with correct paths for: $category"
    }
}

Write-Host "Backup creation completed with correct image paths!"
