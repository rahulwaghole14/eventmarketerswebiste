...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Convert image paths to relative paths for static export
  // Category pages are at /category/[slug], so depth is 2
  const currentRoute = `/category/${slug}`;
  const convertedImages = item.images ? getRelativeImagePaths(item.images, currentRoute) : undefined;
  const convertedTabs = item.tabs ? item.tabs.map(tab => ({
    ...tab,
    images: getRelativeImagePaths(tab.images, currentRoute)
  })) : undefined;
        ...
        <div className={`relative py-16 overflow-hidden bg-gradient-to-br ${item.gradient}`}>
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 md:px-8 lg:px-12">
            ...
            <div className="relative mt-0">
              {item.type === "tabbed" && convertedTabs && (
                <TabbedImageCarousel tabs={convertedTabs} interval={2000} />
              )}
              {item.type === "regular" && convertedImages && (
                <HorizontalImageCarousel images={convertedImages} interval={2000} />
              )}
            </div>
          </div>
        </div>
